...
A collection of Data Structures & Algorithms solutions written in **Python 3**, based on LeetCode problems.

Each file contains a problem description, approach notes, time/space complexity analysis, and example input/output. Every file is also directly runnable via `python <file>.py` (sample test cases live in the `__main__` block).
...
**Total:** 22 solutions · 13 Easy · 9 Medium
...
Python 3.8+ is recommended. No external dependencies — only the standard library.

## 🤝 Contributing

Pull requests and new solution suggestions are welcome. When adding a new solution:

1. Place the file in the appropriate category folder (snake_case filename).
2. Include problem description, approach, complexity analysis, and an example at the top of the file.
3. Run at least one sample test inside the `__main__` block.
4. Update the category README and the main README table.
